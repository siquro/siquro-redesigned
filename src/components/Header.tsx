    ...
    return (
        <header className="nav-padding relative z-50">
            <div className="flex flex-row  md:flex-wrap lg:flex-nowrap items-center justify-between py-8 bg-primaryDark dark:bg-transparent px-10 rounded-b-[15px]">
                <Link href="/" className="w-[132px] md:w-[166px] xl:w-[200px]">
                ...
                <button className="hidden md:block mr-5">
                ...
                {burgerActive && <div className="w-full h-screen bg-[#1E1E1E] absolute top-0 left-0 flex flex-col gap-4 p-5 z-[100]">
                    <div className="flex flex-row justify-between items-center">
                        ...
                        <div className="flex flex-row gap-4">
                            <Swicher small={true} />
                            <div className="text-primaryDark dark:text-primaryLight w-[29px] h-[19px] bg-red-300 mb-0" />
                        </div>



                        <Image className="block cursor-pointer" src="/close.svg" width={16} height={16} alt="cancel" onClick={() => {
                            setBurgerActive(false)
                        }} />
                    </div>

                    <div className="w-full">
                        <input type="text" placeholder="SEARCH" className="border-0 rounded-[20px] py-3 px-4 bg-[#0F0F0F] w-full font-kodchasan text-primaryLight dark:text-primaryLight placeholder:text-primaryLight placeholder:text-[13px]" />
                    </div>

                    <nav className="flex flex-col gap-1">
                        <Link href={"/"} className={`link_mob py-4 ${router.pathname === '/' ? 'active_mob' : ''}`}>Home</Link>
                        <Link href={"/solutions"} className={`link_mob py-4 ${router.pathname === '/solutions' ? 'active_mob' : ''}`}>Solutions</Link>
                        <Link href={"/services"} className={`link_mob py-4 ${router.pathname === '/services' ? 'active_mob' : ''}`}>Services</Link>
                        <Link href={"/company"} className={`link_mob py-4 ${router.pathname === '/company' ? 'active_mob' : ''}`}>Company</Link>
                        <Link href={"/careers"} className={`link_mob py-4 ${router.pathname === '/careers' ? 'active_mob' : ''}`}>Careers</Link>
                        <Link href={"/contacts"} className={`link_mob py-4 ${router.pathname === '/contacts' ? 'active_mob' : ''}`}>Contact Us</Link>
                    </nav>
